...
Lógica principal e orquestração

Matemática

SymPy

Cálculo diferencial simbólico e resolução de equações

Frontend

Streamlit

Interface do usuário e visualização de dados

Backend/DB

PostgreSQL

Armazenamento persistente do histórico de decisões

Análise

Pandas/NumPy

Manipulação de datasets e estatística descritiva
...
Crie um novo banco de dados chamado estoque_opt.

A senha configurada no projeto é 1234 (se a sua for diferente, altere no arquivo main.py).
...
python -m streamlit run app_estoque.py
...
Importação: O sistema define símbolos (Q, D, S, etc.) usando sp.symbols.

Modelagem: A função de custo total $TC$ é construída como uma expressão Python.

Derivação: Utilizamos sp.diff(TC, Q) para obter a derivada primeira.

Resolução: sp.solve(dTC, Q) encontra as raízes da equação (pontos críticos).

Esta abordagem garante precisão absoluta, evitando erros de arredondamento comuns em métodos numéricos tradicionais.
...
Exemplo de CSV compatível (demand_forecasting.csv):
